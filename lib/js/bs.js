...
const bsOptions = {
    ...
    rewriteRules: [
         ...
         */
        {
            ...
            replace: (_match, capturedPath) => capturedPath || '/',
        },
    ],
};
/**
...
const buildOptions = () => {
    ...
    if (bsconfig.enable.startPath)
        opt.startPath = bsOptions.startPath;
    if (bsconfig.enable.https)
        opt.https = bsOptions.https;
    const rules = [];
    if (bsconfig.enable.ssiInclude)
        rules.push(bsOptions.rewriteRules.find((r) => r.id === 'ssiInclude'));
    if (bsconfig.enable.rewriteBaseUrl)
        rules.push(bsOptions.rewriteRules.find((r) => r.id === 'rewriteBaseUrl'));
    if (rules.length > 0)
        opt.rewriteRules = rules;
    return opt;
};
/**
 ...
 */
export const watchServer = (onReady) => {
